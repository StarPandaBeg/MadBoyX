...
def run_process(ngrok_path, args):
    ...
    subprocess.call(start, shell=False)
...
def _start_process(pyngrok_config):
    ...
    popen_kwargs = {"stdout": subprocess.PIPE, "universal_newlines": True, "shell": True}
    if os.name == "posix":
    ...
    proc = subprocess.Popen(start, **popen_kwargs, startupinfo=startupinfo)
